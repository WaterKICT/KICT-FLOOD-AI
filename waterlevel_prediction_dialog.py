...
class WaterlevelPredictionDialog(QtWidgets.QDialog, FORM_CLASS):
                ...
                if ((strTableNm=="rainfall") or (strTableNm=="watershed")):
                    ...
                    for i in range(len_pd_final):
                        ...
                        if (len(rowdata) == 4):
                            ...
                            if ((dt_data == '') or (dt_data == 'nan')):
                                dt_data = "NULL"
                            if ((mi_data == '') or (mi_data == 'nan')):
                                mi_data = "NULL"
                            if ((oi_data == '') or (oi_data == 'nan')):
                                oi_data = "NULL"
                    ...
                    print(strObsId, " TIME_MODIFY")

                    # 데이터 중복 제거
                    pd_removeDuplicate = DATA_QM.REMOVE_DUPLICATE(pd_timeModify)
    ...
    def SaveData_fileDQM(self):
        ...
        for row in range(self.tbl_showData.rowCount()):
            ...
            # 데이터 날짜누락 보간 (원본파일의 시작일 00시00분 부터 종료일 23시50분 까지)
            pd_timeModify = DATA_QM.TIME_MODIFY(pd_removeComma)
            print(strObsId, " TIME MODIFY")

            # 데이터 중복 제거
            pd_removeDuplicate = DATA_QM.REMOVE_DUPLICATE(pd_timeModify)
            print(strObsId, " REMOVE_DUPLICATE")

            pd_nullToZero = None        # 결측치제거(mi_data)
            pd_removeOutlier = None     # 이상치제거(oi_data)

            # 강우데이터일경우, 결측치체크시(원천/결측/결측), 결측치미체크시(원천/원천/원천)
            if (strTableNm == "rainfall"):
                ...
                # 결측치체크시, 원천/결측/결측
                if (bRemoveMissingVAL == True):
                    mi_data = pd_nullToZero.iloc[:, 1]
                    pd_file = pd.concat([dt_data, mi_data, mi_data], axis=1)
                # 결측치미체크시, 원천/원천/원천
                else:
                    mi_data = dt_data.iloc[:, 1]
                    pd_file = pd.concat([dt_data, mi_data, mi_data], axis=1)
                ...
                if (strTableNm == "waterlevel"):
                    if ((bRemoveOutlier == True) or (bRemoveMissingVAL == True)):
                        ...
                        if (bRemoveMissingVAL == True):
                            # 결측치보간 : Null값을 처리안함 (pd_3hrInterpolation = mi_data)
                            pd_nullToZero = pd_3hrInterpolation
                        else:
                            pd_nullToZero = pd_removeDuplicate
    ...
    def Search_OpenAPIData(self):
        ...
        if ipaddress == "127.0.0.1":
            QMessageBox.warning(self, 'Error Message', "인터넷 연결이 올바르지 않습니다.", QMessageBox.Ok)
            return
        ...
        if response.status_code == 200:
            ...
            if (idxDataType == 5):
                for errData in data['result'].keys():
                    if errData == "error":
                        ...
                        QMessageBox.warning(self, 'Error Message', data['result'][errData], QMessageBox.Ok)
                        return False
                ...
                for errData in data.keys():
                    if errData == "message":
                        self.tbl_apiData.setRowCount(0)
                        self.tbl_apiData.setColumnCount(0)
                        QMessageBox.warning(self, 'Error Message', data[errData], QMessageBox.Ok)
                        return False
            ...
            QMessageBox.warning(self, 'Error Message', errMsg, QMessageBox.Ok)
            return False
    ...
    def CheckOpenAPI_SearchDataInputValue(self):
        ...
        if self.cb_apiObsCodeType.currentIndex() == 0:
            ...
            strObsId = self.cb_apiObsCode.currentText()
        if (strObsId == ""):
            QMessageBox.warning(self, 'Error Message', "조회할 관측소 번호를 입력해주세요.", QMessageBox.Ok)
            return False

        # 2. 조회기간이 올바른지(시작일이 종료일보다 크거나 같은지)
        if (self.dt_apiStartDt.date() > self.dt_apiEndDt.date()):
            QMessageBox.warning(self, 'Error Message', "조회 시작일이 종료일보다 클 수 없습니다.", QMessageBox.Ok)
            return False
        ...
        if (idxDataType == 5):
            if (start != end):
                QMessageBox.warning(self, 'Error Message', "조회 기간은 하루(1일)까지만 가능합니다.", QMessageBox.Ok)
                return False
        else:
            if (dateEnd.date() > dateMonth.date()):
                QMessageBox.warning(self, 'Error Message', "조회 기간은 1개월까지만 가능합니다.", QMessageBox.Ok)
                return False
    ...
    def Save_OpenAPIData(self):
        ...
        if self.tbl_apiData.rowCount() == 0:
            QMessageBox.warning(self, 'Error Message', "저장할 데이터가 없습니다.", QMessageBox.Ok)
            return
    ...
    def btn_getSelectedFeatureTarget(self):
        ...
        if (len(selectLyr) > 1):
            QMessageBox.warning(self, 'Error Message', "Target Point는 한개만 선택되어야 합니다.", QMessageBox.Ok)
    ...
    def getSeletedFeature(self, lyrName):
        ...
        if len(selectFeatures) == 0:
            QMessageBox.warning(self, 'Error Message', "선택된 객체가 없습니다.", QMessageBox.Ok)
    ...
    def CheckDatasetInfo(self):
        ...
        if isSelectPoint == False:
            QMessageBox.warning(self, 'Error Message', "조회할 Target 지점 정보를 설정해주세요.", QMessageBox.Ok)
            return False
        ...
        if nRefCount == 0:
            QMessageBox.warning(self, 'Error Message', "조회할 Reference 지점 정보를 설정해주세요.", QMessageBox.Ok)
            return False
        ...
        if self.rb_dataset_periodDate.isChecked():
            ...
            if start <= end:
                isPeriod = True
        if isPeriod == False:
            QMessageBox.warning(self, 'Error Message', "조회할 기간 정보를 설정해주세요.", QMessageBox.Ok)
            return False

        # 4. 전처리옵션 설정체크
        if (self.ckb_ds_preProcessing_noRain.isChecked()):
            if (self.txtBox_ds_preProcessing_noRain.text().strip()==''):
                QMessageBox.warning(self, 'Error Message', "전처리옵션 무강우사상삭제 시간정보를 다시 확인해주세요.", QMessageBox.Ok)
                return False
            else:
                ...
                try:
                    ...
                    QMessageBox.warning(self, 'Error Message', "전처리옵션 무강우사상삭제 시간정보를 다시 확인해주세요.\n(예시 - 전후 8시간일 경우 8 로 표기)", QMessageBox.Ok)
                    return False

        if (self.sp_ds_preProcessing_noRainVal.isChecked()):
            if (self.txtBox_ds_preProcessing_accRainHour.text().strip()==''):
                QMessageBox.warning(self, 'Error Message', "전처리옵션 누적강우 생성정보를 다시 확인해주세요.", QMessageBox.Ok)
                return False
            else:
                ...
                for tmpNumber in tmpAccRainTerm:
                    try:
                        ...
                        QMessageBox.warning(self, 'Error Message', "전처리옵션 누적강우 입력정보를 다시 확인해주세요.\n(예시 - 6시간,8시간일 경우 6,8 로 표기)", QMessageBox.Ok)
                        return False
        ...
        if (self.ckb_ds_preProcessing_month.isChecked()):
            if (self.rb_ds_preProcessing_month.isChecked()):
                ...
                if int(stdMonth) > int(endMonth):
                    QMessageBox.warning(self, 'Error Message', "전처리옵션 기간필터링 정보를 다시 확인해주세요.", QMessageBox.Ok)
                    return False
        ...
        if self.rb_ds_leadtimeTimeseries.isChecked():
            isLeadtime = True
        if isLeadtime == False:
            QMessageBox.warning(self, 'Error Message', "조회할 리드타임 정보를 설정해주세요.", QMessageBox.Ok)
            return False
    ...
    def MakeDataset(self):
        ...
        if (isDataResultOK == False):
            ...
            QMessageBox.warning(self, 'Error Message', "데이터의 갯수가 관측소별로 다릅니다.\n\n" + txtDataInfo, QMessageBox.Ok)
            return
        ...
        if (isLeadResultOK == False):
            ...
            QMessageBox.warning(self, 'Error Message', "Target 관측소의 자료 갯수와 Leadtime 갯수가 다릅니다.\n\n" + txtDataInfo, QMessageBox.Ok)
            return
        ...
        nRainIndex = 0
        if (bNoRainDelete):
            nNoRainRange = int(self.txtBox_ds_preProcessing_noRain.text().strip())
    ...
    def btn_saveDatasetFunc(self):
        ...
        if self.tab_ds_result.currentIndex() == 0:  # 데이터셋원본
            ...
            buttonReply = QMessageBox.warning(self, 'Warning Message',
    ...
    def checkInputParameter(self):
        ...
        if datafilepath == "":
            QMessageBox.warning(self, 'Error Message', "데이터셋 파일을 선택해 주세요.", QMessageBox.Ok)
            return "None"

        # 1-1. 데이터셋에 Null값이 포함되었는지,
        global isMakeModelDataNull
        if isMakeModelDataNull == True:
            QMessageBox.warning(self, 'Error Message', "데이터셋 파일에 NULL값이 포함되어 있습니다.", QMessageBox.Ok)
            return "None"

        # 2.모형제작 저장경로(폴더) 설정되었는지,
        modelpath = self.txtBox_dl_saveParamPath.text()
        if modelpath == "":
            QMessageBox.warning(self, 'Error Message', "모형을 저장할 폴더 경로를 선택해 주세요.", QMessageBox.Ok)
            return "None"

        # 3.모형제작 모델명 설정되었는지,
        modelname = self.txtBox_dl_modelName.text()
        if modelname == "":
            QMessageBox.warning(self, 'Error Message', "모델명을 입력해 주세요.", QMessageBox.Ok)
            return "None"
        ...
        if (len(hiddenlayerunit) > 0 and hiddenlayerunit[0] != '' and int(hiddenlayerunit[0]) > 0):
            ...
            if hiddenlayercount != len(hidden_layer_unit):
                QMessageBox.warning(self, 'Error Message', "Hidden Layer 갯수에 맞게 입력해 주세요.", QMessageBox.Ok)
                return "None"
        else:
            QMessageBox.warning(self, 'Error Message', "Hidden Layer 갯수에 맞게 입력해 주세요.", QMessageBox.Ok)
            return "None"
        ...
        if useCol_checked == False:
            QMessageBox.warning(self, 'Error Message', "모형제작에 사용할 데이터 컬럼을 선택해주세요.", QMessageBox.Ok)
            return "None"
        ...
        if leadtime_checked == False:
            QMessageBox.warning(self, 'Error Message', "모형제작에 사용할 리드타임 컬럼을 선택해주세요.", QMessageBox.Ok)
            return "None"
    ...
    def checkModelParam_load(self):
        ...
        if selectmodelpath == "":
            QMessageBox.warning(self, 'Error Message', "모형 폴더 경로를 선택해 주세요.", QMessageBox.Ok)
            return "None"

        # 3.예측수행결과를 저장할 경로가 선택되었는지(폴더),
        modelpath = self.txtBox_saveModelPath.text()
        if modelpath == "":
            QMessageBox.warning(self, 'Error Message', "예측수행결과를 저장할 폴더 경로를 선택해 주세요.", QMessageBox.Ok)
            return "None"

        # 4.예측수행결과 폴더명을 입력하였는지,
        modelname = self.txtBox_saveModelName.text()
        if modelname == "":
            QMessageBox.warning(self, 'Error Message', "예측수행결과명을 입력해 주세요.", QMessageBox.Ok)
            return "None"
        ...
        if sel_status == False:
            QMessageBox.warning(self, 'Error Message', "예측수행에 사용할 데이터를 선택해주세요.", QMessageBox.Ok)
            return "None"

        # 6. 데이터컬럼정보와 모델의 데이터컬럼정보의 갯수가 일치하는지,
        md_col = self.txtBox_load_dl_datainfo.text().split(',')
        if (len(sel_col) != len(md_col)):
            QMessageBox.warning(self, 'Error Message', "모델에 사용된 데이터와 예측수행에 사용할 데이터의 갯수가 다릅니다.", QMessageBox.Ok)
            return "None"
        ...
        if collectData_flag == False:
            QMessageBox.warning(self, 'Error Message', "모델에 사용된 데이터와 예측수행에 사용할 데이터의 종류가 다릅니다.", QMessageBox.Ok)
            return "None"
        ...
        if lead_status == False:
            QMessageBox.warning(self, 'Error Message', "예측수행에 사용할 리드타임을 선택해주세요.", QMessageBox.Ok)
            return "None"

        # 9. 선택한리드타임과 모델에서 사용한 리드타임이 같은지,
        md_lead = self.txtBox_load_dl_leadtimeinfo.text().split(',')
        if (len(lead_col) != len(md_lead)):
            QMessageBox.warning(self, 'Error Message', "모델에 사용된 리드타임과 예측수행에 사용할 리드타임의 갯수가 다릅니다.", QMessageBox.Ok)
            return "None"
        ...
        for cIdx in range(len(md_lead)):
            ...
            if (model_time != dataset_time):
                ...
                break
        if collectData_flag == False:
            QMessageBox.warning(self, 'Error Message', "모델에 사용된 리드타임과 예측수행에 사용할 리드타임의 종류가 다릅니다.", QMessageBox.Ok)
            return "None"
    ...
    def btn_getSelectedRealtimeFeatureTarget(self):
        ...
        if len(selectLyr) == 1:
            ...
            QMessageBox.warning(self, 'Error Message', "Target Point가 선택되지 않았습니다.", QMessageBox.Ok)
        else:
            QMessageBox.warning(self, 'Error Message', "Target Point가 선택되지 않았습니다.", QMessageBox.Ok)
    ...
    def checkRealtimeModelParam(self, stdTime):
        ...
        if selectmodelpath == "":
            QMessageBox.warning(self, 'Error Message', "실시간예측에 사용할 예측모형을 선택해 주세요.", QMessageBox.Ok)
            return "None"

        # 2.실시간예측수행결과를 저장할 경로가 선택되었는지(폴더),
        modelpath = self.txtBox_saveRealtimeModelPath.text()
        if modelpath == "":
            QMessageBox.warning(self, 'Error Message', "실시간예측 수행결과를 저장할 폴더 경로를 선택해 주세요.", QMessageBox.Ok)
            return "None"
        ...
        if str(strModelData) != strDatasetData:
            QMessageBox.warning(self, 'Error Message', "데이터셋 파일과 모델에서 사용하는 데이터의 종류가 다릅니다.", QMessageBox.Ok)
            return "None"
        ...
        if (dsDatadim == datadim_count) and (dsOutputdim == outputdim_count):
            ...
            QMessageBox.warning(self, 'Error Message', "데이터셋 파일과 모델에서 사용하는 데이터의 갯수가 다릅니다.", QMessageBox.Ok)
            return "None"
    ...
    def get_realtimeDataset(self, stdTime):
        ...
        # 조회결과가 없으면 리턴
        if len(resultset_now) == 0:
            QMessageBox.warning(self, 'Information Message', "조회된 데이터가 없습니다.", QMessageBox.Ok)
            return "None"
        ...
        for idxs in range(len(resultset_now)):
            if idxs == 0:
                ...
                isValue = len(resultset_now[idxs])
                if isFirstValue != isValue:
                    QMessageBox.warning(self, 'Information Message', "데이터의 갯수가 관측소별로 다릅니다.", QMessageBox.Ok)
                    return "None"
...
class DataQualityManagement():
    ...
    def TIME_EXPRESSION_MODIFY(self):
        ...
        Data = pd.read_csv(self.origin_file, header=None, dtype={0:object, 1:object})
        FILE_1 = pd.DataFrame(Data)
        check_data_1shape_0 = FILE_1.shape[0]
        for i in range(check_data_1shape_0):  # 파일 내부 데이터의 이터레이션을 진행함.

            # remove_comma작업 추가(하나로 합침)
            FILE_1.iloc[i, 1] = str(FILE_1.iloc[i, 1]).replace(',', '')
            ...
            if date_num[8:13] == str(2400):
                ...
                current_time_1 = datetime(int(date_num_1[0:4]), int(date_num_1[4:6]),
                                                   int(date_num_1[6:8]),
    ...
    def TIME_MODIFY(self, pdData):
        ...
        tmpStartDay = str(RAW_DATA_1.iloc[0, 0])
        tmpEndDay = str(RAW_DATA_1.iloc[-1, 0])
        startDay = "{0}-{1}-{2} 00:00".format(tmpStartDay[0:4], tmpStartDay[4:6], tmpStartDay[6:8])
        endDay = "{0}-{1}-{2} 23:50".format(tmpEndDay[0:4], tmpEndDay[4:6], tmpEndDay[6:8])

        # date_time_pandas (10m term)
        pd_tmp = pd.date_range(startDay, endDay, freq='10T')
        tmpDates = pd.DataFrame(pd_tmp.strftime('%Y%m%d%H%M'))

        tmpDatas = RAW_DATA_1.astype({0: 'str'})
        Vlookup_Table = tmpDates.astype({0: 'str'})

        df = pd.merge(Vlookup_Table, tmpDatas, how='left', left_on=0, right_on=0)
        Data_Frames_1 = df

        return Data_Frames_1
    ...
    def MINUS_TO_ZERO(self, pdData):
        ...
        for i in range(TEST_FILE_2.shape[0]):
            if ((str(TEST_FILE_2.iloc[i, 1]).strip() != '') and (float(TEST_FILE_2.iloc[i, 1]) < 0)):
        ...
        while (start_idx + 20) < MATRIX_row_num:
            ...
            if start_idx == 0:
                ...
                if WINDOW_DATA_2.isnull().sum() == 19:  # 전체 또는 일부가 WINDOW가 NULL
                    # print("  4 - ELSE")
                    tmp_data1 = WHOLE_DATA_1.iloc[start_idx - 1]
                    tmp_data2 = WHOLE_DATA_1.iloc[start_idx + 20]
                    data1 = 0.0
                    data2 = 0.0
                    if str(type(tmp_data1)) == "<class 'str'>":
                        data1 = float(tmp_data1)
                    else:
                        data1 = tmp_data1
                    if str(type(tmp_data2)) == "<class 'str'>":
                        data2 = float(tmp_data2)
                    else:
                        data2 = tmp_data2

                    #if np.isnan(WHOLE_DATA_1.iloc[start_idx - 1]) and np.isnan(WHOLE_DATA_1.iloc[start_idx + 20]):  # 3시간만 null 이기 때문에 보간 시작
                    if np.isnan(data1) and np.isnan(data2):  # 3시간만 null 이기 때문에 보간 시작
                        # print("  5 - ELSE")
        ...
        for i in range(len_datafile):
            ...
            if (len(rowdata) == 2):
                ...
                if nRowIndex >= ((6 * 12) - 1):
                    ...
                    myData = [float(i) if str(i).strip()!='' else np.NAN for i in myDatas]
